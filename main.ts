    ...
    if (!diagram) {
      ...
      if (line.trim() === ":::") {
        if (diagram[0].match(/^\s*graph\s+/)) {
          ...
          if (!diagramsDir.exists || !diagramsDir.at(diagramImage).exists) {
            ...
            try {
              const command = [
                ...
                sourcePath.relPath,
                "-o",
                diagramsDir.at(diagramImage).relPath,
                options,
              ].join(" ");
          ...
          lines.push(
            `![${diagramImage}](./${diagramsDir.name}/${diagramImage})`
          );
        }
        diagram = undefined;
      } else {
...
function addSnippet(
  ...
  let snippetLines = getDateFile(sourceDir, blogDate)
    .text.split("\n")
    .map((l) => l.trim());
  ...
  parts.push(
    convertLinks(getLink, snippetLines.join(EOL), rootPath, diagramOwner)
  );
  parts.push(`[More...](${rootPath}/${formatDate(blogDate).path})`);
}

const images = Object.fromEntries(
  [".png", ".jpg", ".jpeg", ".gif", ".svg"].map((x) => [x, true])
);
...
function generate() {
  ...
  for (const year of sourceDir.contents.filter((x) => x.isDirectory)) {
    for (const month of year.contents.filter((x) => x.isDirectory)) {
      for (const file of month.contents.filter((x) => !x.isDirectory)) {
        const yearVal = parseInt(year.name, 10);
        const monthVal = parseInt(month.name, 10);

        if (file.extension === ".md") {
          blogDates.push(new Date(yearVal, monthVal - 1, parseInt(file.title)));
        }

        if (images[file.extension]) {
          const f = formatDate(new Date(yearVal, monthVal - 1, 1));
          const imageTarget = prettyDir.asDirectory
            .at(f.year)
            .asDirectory.at(f.month)
            .asDirectory.at(file.name);

          const sourceModified = file.stat.mtime;
          const targetModified = imageTarget.exists
            ? imageTarget.stat.mtime
            : 0;
          if (sourceModified > targetModified) {
            file.copyTo(imageTarget);
          }
        }
      }
    }
  ...
  for (const blogDate of blogDates) {
    ...
    findLinks(
      getDateFile(sourceDir, blogDate).text,
      undefined,
      (linkText, lineNumber) => {
  ...
  for (let i = 0; i < blogDates.length; i++) {
    ...
    const content = convertLinks(
      linkMap,
      getDateFile(sourceDir, today).text,
      "../..",
      target
    );
  ...
  const homePage = [sourceDir.at("README.md").text];
  ...
  homePage.push(
    linkTopics.map((l) => `[${l.text}](Blog/links/${l.id})`).join(" | ")
  );
  ...
  const dayHeader =
    "|" +
    dayNames.map((n) => n.substr(0, 2)).join("|") +
    "|" +
    EOL +
    "|" +
    dayNames.map(() => "--").join("|") +
  ...
  for (let year = firstYear; year <= lastYear; year++) {
    ...
    for (let month = 0; month < monthNames.length; month++) {
      ...
      if (date < blogDates[0] || date > blogDates[blogDates.length - 1]) {
        continue;
      }
